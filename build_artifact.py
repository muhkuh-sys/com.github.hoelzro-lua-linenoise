        ...
        if tPlatform['cpu_architecture'] == tPlatform['host_cpu_architecture']:
            ...
            astrCMAKE_COMPILER = [
                '-DCMAKE_TOOLCHAIN_FILE=' + os.path.join(
                    ...
                    'cmake',
                    'toolchainfiles',
                    'toolchain_ubuntu_riscv64.cmake'
                )
            ]
            astrCMAKE_PLATFORM = [
                '-DJONCHKI_PLATFORM_DIST_ID=%s' %
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lua-linenoise-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lua-linenoise-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-lua-linenoise-*.xml".')
